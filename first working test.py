    ...
    if filename.endswith(".yaml"):
        with open(os.path.join("config", filename), "r") as f:
            ...
            if "handler" in config:
                ...
                if handler == "Wikidata":
                    # Handle Wikidata-based configuration files
                    source_lang = config["source"]["from"]
                    target_langs = [lang["to"] for lang in config["target"]]
                    engine = "Wikidata"
                    preferred_engine = False
                elif handler == "Meta":
                    # Handle Meta-based configuration files
                    source_lang = filename.split(".")[0]
                    target_langs = config["languages"]
                    engine = config["engine"]
                    preferred_engine = config.get("preferred_engine", False)
                # else:
                    raise ValueError("Unsupported handler: {handler}")
            else:
                # Handle standard configuration files
                # Check if config is a list
                if isinstance(config, list):
                    # Handle list-based configuration files
                    source_lang = filename.split(".")[0]
                    target_langs = config
                    engine = "Apertium"
                    preferred_engine = False
                    ...
                    engine = config.get("engine", "Apertium")
                    preferred_engine = config.get("preferred_engine", False)
